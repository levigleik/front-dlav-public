...
import TabsFunc from '@/app/(notlogged)/(cleaner)/tinturaria/(logged)/novo/(new)/tabs'
...
const CartTab = () => {
  ...
  const { setTab, location } = useRolTinturiaHook()
  ...
  const handleSubimt = () => {
    ...
    mutatePost({
      ...
      .then((data) => {
        ...
        setTimeout(() => {
          router.push('/tinturaria/acompanhamento')
          clearCart()
        }, 500)
      })
      .catch((error: any) => {
  ...
  return (
    <div className="flex w-full flex-col gap-4">
      ...
      <div className="flex items-center gap-4">
        <span className="">
          Cliente:{' '}
          <b>{profileCleaner?.fantasyName ?? profileCleaner?.corporateName}</b>
        </span>
      </div>
      {cart?.products && (
        <Accordion

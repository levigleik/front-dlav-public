...
const ProductTab = ({
  ...
  const { search, groupId, setTab } = useRolTinturiaHook()
  const { setProductTemp: setProduct } = useCartTinturaria()

  const { data: dataGetProducts, isLoading: loadingGetProduct } = useQuery({
    queryFn: ({ signal }) =>
      getData<
        PriceListProps<
          GroupProps<ProductDefaultProps, any>,
          ProductDefaultProps,
          any
        >[]
      >({
        url: 'priceList/web',
        signal,
        query: `orderBy.name=asc&&where.type=web&&include.groups.include.products=true`,
      }),
    queryKey: ['product-get'],
    select: (priceList) => {
      const products = priceList
        .map((priceList) =>
          priceList.groups!.map((group) => group.products!).flat(),
        )
        .flat()
        .filter((product) => product)
        .map((product) => ({
          ...product,
          // randomize the price with one decimal

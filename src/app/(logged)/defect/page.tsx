...
import { columnsDefects } from './constants'
...
export default function Defect() {
  ...
  const deleteItem = (id: number) => {
    mutateDelete({
      ...
      .then(() => {
        toast.success('Defeito deletado com sucesso')
        refetch()
      })
      .catch((err) => {
        toastErrorsApi(err)
  ...
  const finalColumns: ColumnProps<DefectProps<any>>[] = [
    ...columnsDefects,
    {
      uid: 'actions',
      label: 'Ações',
        ...
        <div className="relative flex cursor-pointer items-center justify-end gap-5">
          ...
          >
            <Button
              ...
              onClick={() => router.push(`/defect/${item.id}`)}
            >
          ...
          {(onClose) => (
            <>
              <ModalHeader className="mt-4 flex flex-col gap-1">
                Tem certeza que deseja deletar o defeito?
              </ModalHeader>
              <ModalBody>
                <div className={'flex flex-col gap-2 text-default-600'}>
                  Você está prestes a deletar o defeito, deseja continuar?
                </div>
              </ModalBody>
              <ModalFooter>

...
import { DeleteData, PostData, PutData } from '@/types'
import {
  ProductRolGroupFormProps,
  ProductRolGroupFormSendProps,
} from '@/app/(logged)/rol/cart/(tabs)/types'
import { ProductRolGroupProps } from '@/types/productRolGroup'
import { toast } from 'react-toastify'

export default function SubdivisionTab() {
  const { cart, setCart, isViewOnly } = useCart()
  ...
  const {
    data: dataGetProductRolGroup,
    isLoading: loadingGetProductRolGroup,
    refetch,
  } = useQuery({
    queryFn: ({ signal }) =>
      getData<ProductRolGroupProps[]>({
        url: 'productRolGroup',
        signal,
        query: `include.products=true${id ? `&&where.rolId=${id}` : ''}`,
      }),
    queryKey: ['product-rol-group-get', id],
  ...
  const { mutateAsync: mutatePost, isPending: loadingPost } = useMutation({
    mutationFn: async (val: PostData<ProductRolGroupFormSendProps>) =>
      postData<ProductRolGroupProps, ProductRolGroupFormSendProps>(val),
    mutationKey: ['product-rol-group-post'],
  })

  const { mutateAsync: mutatePut, isPending: loadingPut } = useMutation({
    mutationFn: async (val: PutData<ProductRolGroupFormSendProps>) =>
      putData<ProductRolGroupProps, ProductRolGroupFormSendProps>(val),
    mutationKey: ['product-rol-group-put'],
  })

  const { mutateAsync: mutateDelete, isPending: loadingDelete } = useMutation({
    mutationFn: async (val: DeleteData) =>
      deleteData<ProductRolGroupProps>(val),
    mutationKey: ['product-rol-group-delete'],
  })
  ...
  const getAvailableProducts = (currentIndex: number) => {
    ...
    const selectedProductsInPreviousLots = fields
      ...
      .flatMap((field) => field.productRolIds)
  ...
  const onSubmit = async (data: ProductRolGroupFormProps) => {
    const parseData = {
      fields: data.fields.map((item) => ({
        ...item,
        productRolIds: item.productRolIds.map((id) => parseInt(id, 10)),
        productStatusId: parseInt(item.productStatusId, 10),
      })),
    }
    parseData.fields.forEach((item) => {
      mutatePost({
        url: '/productRolGroup',
        data: { ...item, rolId: cart?.id! },
      })
        .then((dataPost) => {
  ...
  useEffect(() => {
    if (dataGetProductRolGroup?.length && id !== undefined) {
      remove()
      dataGetProductRolGroup.forEach((item) => {
        append({
          productRolIds: item.products.map((product) => String(product.id)),
          productStatusId: String(item.productStatusId),
          name: item.name,
          id: String(item.id),
        })
      })
    }
  }, [append, dataGetProductRolGroup, id, remove])
                ...
                {idSubdivision && (
                  ...
                  >
                    <Button
                      ...
                      className="w-fit rounded-full text-main-white"
                      onClick={() => {
                        setItemDelete(Number(idSubdivision))
              ...
              <>
                ...
                <ModalBody>
                  <div className={'flex flex-col gap-2 text-default-600'}>
                    Você está prestes a deletar o lote, deseja continuar?
                  </div>
                </ModalBody>
                <ModalFooter>

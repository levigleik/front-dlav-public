...
const WashControlProductTab = () => {
  const { search, employeeId, setTab } = useRolHook()
  ...
  const { data: dataGetProducts, isLoading: loadingGetProduct } = useQuery({
    queryFn: ({ signal }) =>
      getData<ProductDefaultProps[]>({
        ...
        signal,
        query:
          `orderBy.name=asc&&where.washControlEmployees` +
          `.some.employeeId=${employeeId}` +
          `&&include.washControlEmployees=true` +
          `&&where.washControlEmployees.every.active=true`,
      }),
    queryKey: ['product-get', employeeId],
  })
...
export default WashControlProductTab

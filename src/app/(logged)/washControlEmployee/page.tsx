...
import { columnsWashControl } from '@/app/(logged)/washControlEmployee/constants'
import { ClientProps } from '@/types/client'
import { ProductDefaultProps } from '@/app/(logged)/product/[id]/types'
import { ColumnProps } from 'components/table/types'

export default function WashControl() {
  const { data, isLoading, refetch } = useQuery({
    queryKey: ['wash-control-get'],
    queryFn: ({ signal }) =>
      getData<WashControlEmployeeProps<ClientProps, ProductDefaultProps>[]>({
        url: '/washControlEmployee',
        signal,
        query: 'include.employee=true&&include.product=true',
      }),
  })
  ...
  const { mutateAsync: mutateDelete, isPending: loadingDelete } = useMutation({
    ...
    mutationKey: ['wash-control-delete'],
  })
        ...
        <div className="relative flex cursor-pointer items-center justify-end gap-5">
          ...
          >
            <Button
              ...
              onClick={() => router.push(`/washControlEmployee/${item.id}`)}
            >
